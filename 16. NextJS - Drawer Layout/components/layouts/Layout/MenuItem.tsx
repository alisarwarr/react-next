import ListItem from "@mui/material/ListItem";
import ExpandLessIcon from "@mui/icons-material/ExpandLess";
import ExpandMoreIcon from "@mui/icons-material/ExpandMore";
import { useRouter, usePathname } from "next/navigation";
import { useSelector } from "react-redux";
import { selectDrawer } from "../../../store/drawer/drawerSlice";
import { useDrawerActions } from "../../../store/drawer/drawerAction";

interface MenuItemProps {
  menu: any;
  nestedClass?: string;
  display: boolean;
}

export default function MenuItem({
  menu,
  nestedClass,
  display,
}: MenuItemProps) {
  const router = useRouter();
  const pathname = usePathname();

  const drawerData = useSelector(selectDrawer);
  const textColor = drawerData.textColor;
  const selectedBgColor = drawerData.selectedBgColor;
  const selectedTextColor = drawerData.selectedTextColor;
  const lineOnSelect = drawerData.lineOnSelect;
  const bgOnSelect = drawerData.bgOnSelect;
  const sidebarSmall = drawerData.sidebarSmall;
  const drawer = drawerData.drawer;
  const drawerOpen = drawerData.drawer?.open;

  const {
    setDrawerExpendKey,
    setDrawerExpendKeyAndMenu,
    setDrawerOpen,
    setDrawerActiveMenu,
    setDrawerExpendMenuAndOpen,
  } = useDrawerActions();

  const toggleDrawer = () =>
    drawerOpen ? setDrawerOpen(false) : setDrawerOpen(true);
  
  let colorful = false;
  if(
    drawer?.activeMenu?.url === menu.url
  ) {
    colorful = true
  }
  else if(
    pathname === menu.url
  ) {
    colorful = true
  }

  return (
    <ListItem
      disablePadding
      disabled={menu.disable}
      className="m-2 ml-4 !w-[90%] cursor-pointer pt-2 pb-2 pr-3.5 pl-3"
      sx={{
        color:
        colorful
            ? selectedTextColor
            : textColor,
        borderLeft:
          lineOnSelect &&
          colorful
            ? `4px solid ${selectedTextColor}`
            : `4px solid transparent`,
        backgroundColor:
          bgOnSelect &&
          colorful
            ? selectedBgColor
            : "transparent",
      }}
      onClick={() => {
        setDrawerExpendKey({
          expendKey: menu.name,
        });

        if (menu.disable) {
          return;
        }

        if (menu.name === "logout") {
          // userLogout();
        }

        if (menu?.url) {
          if (menu.url !== drawer.activeMenu) {
            setDrawerActiveMenu(menu);

            router.push(menu.url);

            if (window.innerWidth < 768) {
              setDrawerExpendMenuAndOpen({
                expendMenu: false,
                open: false,
              });
            }
          }
        } else {
          setDrawerExpendKeyAndMenu({
            expendMenu:
              drawer?.expendKey === menu.name ? !drawer?.expendMenu : true,
            expendKey: menu.name,
          });
        }
      }}
    >
      <div className={`flex-1 flex items-center -ml-1 ${nestedClass}`}>
        <span className="mr-3 ml-1">
          {menu.icon(
            colorful
              ? selectedTextColor
              : textColor
          )}
        </span>
        {!sidebarSmall && (
          <span
            className={`mt-0.5 text-xs xl:text-base whitespace-nowrap ${
              !display && `hidden`
            }`}
          >
            {menu.name}
          </span>
        )}
      </div>
      {menu?.options?.length > 0 &&
        !sidebarSmall &&
        (drawer?.expendMenu && menu.name === drawer?.expendKey ? (
          <ExpandLessIcon className="!text-[14px] sm:!text-[18px]" />
        ) : (
          <ExpandMoreIcon className="!text-[14px] sm:!text-[18px]" />
        ))}
    </ListItem>
  );
}
